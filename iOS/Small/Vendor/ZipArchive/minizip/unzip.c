                                                  ...
                                                  uLong commentBufferSize)
{
    ...
    if ((err==UNZ_OK) && (szComment!=NULL))
    {
        ...
        if (lSeek!=0)
        {
            if (ZSEEK64(s->z_filefunc, s->filestream,lSeek,ZLIB_FILEFUNC_SEEK_CUR)==0)
                ;
//                lSeek=0; fix for never read by `Analyze'
            else
                err=UNZ_ERRNO;
        }
        ...
        if ((file_info.size_file_comment>0) && (commentBufferSize>0))
            if (ZREAD64(s->z_filefunc, s->filestream,szComment,uSizeRead)!=uSizeRead)
                err=UNZ_ERRNO;
//        lSeek+=file_info.size_file_comment - uSizeRead; fix for never read by `Analyze'
    }
    else ;
//        lSeek+=file_info.size_file_comment; fix for never read by `Analyze'
...
#if 0  // fix for never read by `Analyze'
